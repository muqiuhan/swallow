...
TEST_CASE(
  "structopt can parse vector positional argument" * test_suite("vector_positional"))
{
  // Vector of ints
  {
...
TEST_CASE(
  "structopt can parse vector of pairs positional argument"
  * test_suite("vector_positional"))
{
  ...
  }
  {
    ...
    REQUIRE(
      arguments.values == std::vector<std::pair<std::string, int>>{{"a", 1}, {"b", 2}});
  }
  {
    auto arguments = structopt::app("test").parse<VectorOfPairs>(
    ...
    REQUIRE(
      arguments.values
      == std::vector<std::pair<std::string, int>>{{"a", 1}, {"b", 2}, {"c", 3}, {"d", 4}});
  }
  {
    bool exception_thrown = false;
    try
...
TEST_CASE(
  "structopt can parse vector of files with an optional argument in the way"
  * test_suite("vector_positional"))
{
  {
