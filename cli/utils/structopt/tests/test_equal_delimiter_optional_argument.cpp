...
TEST_CASE(
  "structopt can parse optional argument with equal delimited value"
  * test_suite("single_optional"))
{
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFoo>(
      std::vector<std::string>{"./main"});
    REQUIRE(arguments.foo.has_value() == false);
  }
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFoo>(
      std::vector<std::string>{"./main", "--foo=bar"});
  ...
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFoo>(
      std::vector<std::string>{"./main", "--foo:bar=baz"});
...
{
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFooBar>(
      std::vector<std::string>{"./main"});
  ...
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFooBar>(
      std::vector<std::string>{"./main", "--foo_bar:baz"});
    REQUIRE(arguments.foo_bar == "baz");
  }
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFooBar>(
      std::vector<std::string>{"./main", "-foo-bar:baz=taz"});
  ...
  {
    auto arguments = structopt::app("test").parse<OptionalArgumentFooBar>(
      std::vector<std::string>{"./main", "-f:=baz"});
    REQUIRE(arguments.foo_bar == "=baz");
  }
}