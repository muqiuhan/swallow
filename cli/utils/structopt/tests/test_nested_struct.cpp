...
{
  {
    auto arguments = structopt::app("test").parse<TopLevelCommand>(
      std::vector<std::string>{"./main", "foo", "15"});
  ...
  {
    auto arguments = structopt::app("test").parse<TopLevelCommand>(
      std::vector<std::string>{"./main", "foo", "--local", "true", "15"});
    REQUIRE(arguments.foo.has_value() == true);
    REQUIRE(arguments.foo.bar == 15);
  ...
  {
    auto arguments = structopt::app("test").parse<TopLevelCommand>(
      std::vector<std::string>{"./main", "foo", "15", "--local", "true"});
    REQUIRE(arguments.foo.has_value() == true);
    REQUIRE(arguments.foo.bar == 15);
    REQUIRE(arguments.foo.local.has_value() == true);
  }
