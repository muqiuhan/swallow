...
TEST_CASE("structopt can parse set positional argument" * test_suite("set_positional"))
{
  // Set of ints
  {
    auto arguments = structopt::app("test").parse<SetIntArgument>(
      std::vector<std::string>{"./main", "1", "2", "3", "1", "4"});
...
{
  {
    auto arguments = structopt::app("test").parse<SetIntArgumentWithOtherFlags>(
  ...
  {
    auto arguments = structopt::app("test").parse<SetIntArgumentWithOtherFlags>(
      std::vector<std::string>{"./main", "1", "2", "3", "4", "5", "3", "--foo"});
    REQUIRE(arguments.value == std::set<int>{1, 2, 3, 4, 5});
    REQUIRE(arguments.foo == true);
  }
  {
    auto arguments = structopt::app("test").parse<SetIntArgumentWithOtherFlags>(
      std::vector<std::string>{"./main", "--foo", "1", "2", "3", "4"});
    REQUIRE(arguments.value == std::set<int>{1, 2, 3, 4});
    REQUIRE(arguments.foo == true);
  }
  {
    ...
    try
      {
        auto arguments = structopt::app("test").parse<SetIntArgumentWithOtherFlags>(
          std::vector<std::string>{"./main", "1", "1", "--foo", "3"});
...
TEST_CASE(
  "structopt can parse set of files with an optional argument in the way"
  * test_suite("vector_positional"))
{
  {
    auto arguments = structopt::app("test").parse<CompilerOptionsForSetOfFiles>(
      std::vector<std::string>{"./main"});
