...
DOCTEST_MSVC_SUPPRESS_WARNING(4626) // assignment operator was implicitly defined as
                                    // deleted
DOCTEST_MSVC_SUPPRESS_WARNING(5027) // move assignment operator was implicitly defined as
                                    // deleted
DOCTEST_MSVC_SUPPRESS_WARNING(5026) // move constructor was implicitly defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4623) // default constructor was implicitly defined as
                                    // deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4640) // construction of local static object is not
                                    // thread-safe
// static analysis
DOCTEST_MSVC_SUPPRESS_WARNING(26439) // This kind of function may not throw. Declare it
                                     // 'noexcept'
DOCTEST_MSVC_SUPPRESS_WARNING(26495) // Always initialize a member variable
DOCTEST_MSVC_SUPPRESS_WARNING(26451) // Arithmetic overflow ...
DOCTEST_MSVC_SUPPRESS_WARNING(26444) // Avoid unnamed objects with custom construction and
                                     // dtr...
...
namespace doctest
{
  ...
  // strings with length of up to 23 chars on the stack before going on the heap - the
  // last byte of the buffer is used for:
  // - "is small" bit - the highest bit - if "0" then it is small - otherwise its "1"
  // (128)
  // - if small - capacity left before going on the heap - using the lowest 5 bits
  // - if small - 2 bits are left unused - the second and third highest ones
  // - if small - acts as a null terminator if strlen() is 23 (24 including the null
  // terminator)
  //              and the "is small" bit remains "0" ("as well as the capacity left") so
  //              its OK
...
#undef DOCTEST_APPROX_PREFIX
  ...
  namespace detail
  {
    ...
    String stringifyBinaryExpr(
      const DOCTEST_REF_WRAP(L) lhs, const char* op, const DOCTEST_REF_WRAP(R) rhs)
    {
...
#define DOCTEST_FORBIT_EXPRESSION(rt, op)                                                \
  ...
  {                                                                                      \
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_PUSH
    // http://stackoverflow.com/questions/39479163 what's the difference between 4018 and
    // 4389
...
#define DOCTEST_BINARY_RELATIONAL_OP(n, op) \
    ...
    DOCTEST_INTERFACE void decomp_assert(
      assertType::Enum at, const char* file, int line, const char* expr, Result result);
...
namespace doctest
{
  ...
  struct DOCTEST_INTERFACE IReporter
  {
    ...
    // doctest will not be managing the lifetimes of reporters given to it but this would
...
#define DOCTEST_TEST_CASE_FIXTURE(c, decorators)                                         \
  DOCTEST_IMPLEMENT_FIXTURE(                                                             \
    DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_),                                             \
    c,                                                                                   \
    DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_),                                              \
    decorators)
...
DOCTEST_MSVC_SUPPRESS_WARNING(4619) // invalid compiler warning
DOCTEST_MSVC_SUPPRESS_WARNING(4996) // The compiler encountered a deprecated declaration
DOCTEST_MSVC_SUPPRESS_WARNING(4267) // 'var' : conversion from 'x' to 'y', possible loss
                                    // of data
DOCTEST_MSVC_SUPPRESS_WARNING(4706) // assignment within conditional expression
DOCTEST_MSVC_SUPPRESS_WARNING(4512) // 'class' : assignment operator could not be
                                    // generated
DOCTEST_MSVC_SUPPRESS_WARNING(4127) // conditional expression is constant
DOCTEST_MSVC_SUPPRESS_WARNING(4530) // C++ exception handler used, but unwind semantics
                                    // not enabled
DOCTEST_MSVC_SUPPRESS_WARNING(4577) // 'noexcept' used with no exception handling mode
                                    // specified
DOCTEST_MSVC_SUPPRESS_WARNING(4774) // format string expected in argument is not a string
                                    // literal
DOCTEST_MSVC_SUPPRESS_WARNING(4365) // conversion from 'int' to 'unsigned',
                                    // signed/unsigned mismatch
DOCTEST_MSVC_SUPPRESS_WARNING(4820) // padding in structs
DOCTEST_MSVC_SUPPRESS_WARNING(4640) // construction of local static object is not
                                    // thread-safe
DOCTEST_MSVC_SUPPRESS_WARNING(5039) // pointer to potentially throwing function passed to
                                    // extern C
...
DOCTEST_MSVC_SUPPRESS_WARNING(5026) // move constructor was implicitly defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4625) // copy constructor was implicitly defined as deleted
DOCTEST_MSVC_SUPPRESS_WARNING(4800) // forcing value to bool 'true' or 'false'
                                    // (performance warning)
// static analysis
DOCTEST_MSVC_SUPPRESS_WARNING(26439) // This kind of function may not throw. Declare it
                                     // 'noexcept'
  ...
  namespace detail
  {
    ...
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
      ...
      void finalizeTestCaseData()
      {
        ...
        if (
          Approx(currentTest->m_timeout).epsilon(DBL_EPSILON) != 0
    ...
    // TODO: figure out if this is indeed necessary/correct - seems like either there
    // still could be a race or that there wouldn't be a race even if using the context
    // directly
...
namespace doctest
{
  namespace
  {
    ...
    // register a reporter with a duplicate name and a different priority but hopefully
...
#define DOCTEST_ITERATE_THROUGH_REPORTERS(function, ...)                                 \
  ...
  namespace
  {
    ...
    // matching of a string against a wildcard mask (case sensitivity configurable) taken
    ...
    // checks if the name matches any of the filters (and can be configured what to do
    // when empty)
    bool matchesAny(
      const char*                name,
  ...
  namespace detail
  {
    ...
    DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4996) // std::uncaught_exception is deprecated
                                                  // in C++17
...
// clang-format on
  ...
  namespace detail
  {
    ...
    // stringify after ContextScope has been destroyed (base class destructors run after
    // derived class destructors). Instead, ContextScope calls this method directly from
    // its destructor.
  ...
  namespace
  {
    ...
    std::ostream& file_line_to_stream(
      std::ostream& s, const char* file, int line, const char* tail = "")
    {
    ...
    struct XmlReporter : public IReporter
    {
      ...
      void test_case_end(const CurrentTestCaseStats& st) override
      {
        xml.startElement("OverallResultsAsserts")
          .writeAttribute(
            "successes", st.numAssertsCurrentTest - st.numAssertsFailedCurrentTest)
    ...
    struct ConsoleReporter : public IReporter
    {
      ...
      void test_run_end(const TestRunStats& p) override
      {
        ...
        s << Color::Cyan << "[doctest] " << Color::None << "assertions: " << std::setw(6)
          ...
          << std::setw(6) << p.numAssertsFailed << " failed" << Color::None << " |\n";
        s << Color::Cyan << "[doctest] " << Color::None
      ...
      void log_assert(const AssertData& rb) override
      {
        ...
        if (
          (rb.m_at & (assertType::is_throws_as | assertType::is_throws_with))
        ...
        if (rb.m_at & assertType::is_throws)
          ...
          (rb.m_at & assertType::is_throws_as) && (rb.m_at & assertType::is_throws_with))
          { //! OCLINT
            s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \""
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                          \
  ...
  {                                                                                      \
    ...
    // parses a comma separated list of words after a pattern in one of the arguments in
    // argv
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                               \
  if (                                                                                    \
    parseOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", &strRes, default)     \
  ...
  p->var = strRes
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "help")
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "version")
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "count")
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-test-cases")
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-test-suites")
    ...
    if (
      parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-reporters")
    ...
    for (auto& curr : testArray)
      {
        ...
        if (
          (p->last < p->numTestCasesPassingFilters && p->first <= p->last)
        ...
        if (p->list_test_suites)
          {
            if (
              (testSuitesPassingFilt.count(tc.m_test_suite) == 0)
              && tc.m_test_suite[0] != '\0')
              {
          ...
          do
            {
              ...
              // exit this loop if enough assertions have failed - even if there are more
              // subcases
              if (
                p->abort_after > 0
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4007) // 'function' : must be 'attribute' - see
                                              // issue #182
