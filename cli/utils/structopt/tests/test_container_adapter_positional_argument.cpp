...
TEST_CASE("structopt can parse stack positional argument" * test_suite("stack_positional"))
{
  // List of ints
  {
    auto arguments = structopt::app("test").parse<StackIntArgument>(
...
TEST_CASE("structopt can parse queue positional argument" * test_suite("queue_positional"))
{
  // List of ints
  {
    auto arguments = structopt::app("test").parse<QueueIntArgument>(
      std::vector<std::string>{"./main", "1", "2", "3"});
    REQUIRE(arguments.value.size() == 3);
...
TEST_CASE(
  "structopt can parse priority_queue positional argument"
  * test_suite("priority_queue_positional"))
{
  // List of ints
  {
    auto arguments = structopt::app("test").parse<PriorityQueueIntArgument>(
      std::vector<std::string>{"./main", "3", "2", "1"});
    REQUIRE(arguments.value.size() == 3);
    REQUIRE(arguments.value.top() == 1);
    arguments.value.pop();
    REQUIRE(arguments.value.top() == 2);
