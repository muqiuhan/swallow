...
let rec read_sexpr stream =
  ...
  then Object.Symbol (Object.string_of_char ch ^ read_symbol stream)
  else if Char.is_digit ch || Char.equal ch '~'
  then (if Char.equal '~' ch then '-' else ch) |> Char.escaped |> read_fixnum stream
  else if Char.equal ch '('
  then read_list stream
  else if Char.is_boolean ch
  then read_boolean stream
  else if Char.equal ch '\''
  then Quote (read_sexpr stream)
  else if Char.equal ch '\"'
  then read_string stream
  else raise (Errors.Syntax_error_exn (Unexcepted_character (Char.escaped ch)))
