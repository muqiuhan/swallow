let stdlib_string =
    ...
    \        (#t (lookup. key (cdr alist)))))\n\n\
     ; esetq takes two parameters: an expression and an environment. It's like \
     our\n\
     ; evalexp.\n\
     (defun eval. (e env)\n\
    \   (letrec (\n\
    ...
    \        ; self-evaluate.\n\
    \        ((atom? e) e)\n\
    \        ; If it's a list (the only alternative to being an atom), check \
     if the\n\
    ...
    \           ; For atom?, eq, car, cdr, and cons, just evaluate the \
     expression\n\
    ...
    \                                       (eval. (caddr e) env)))\n\
    \           ; For cond, it's a wee bit tricker. We get to this function a \
     bit\n\
    \           ; later.\n\
    ...
    \        ; lambda-expresison,\n\
    \        ((eq (caar e) 'lambda)\n\
    \         ; ...evaluate the application of the lambda to the given \
     arguments,\n\
    \         ; evaluating them.\n\
    \         (eval. (caddar e)\n\
    \                (append. (zip. (cadar e)\n\
