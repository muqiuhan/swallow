...
{
  {
    auto arguments = structopt::app("test").parse<ArrayOptionalArgument>(
      std::vector<std::string>{"./main", "--indices", "1", "2", "3"});
    REQUIRE(arguments.indices == std::array<int, 3>{1, 2, 3});
  }
  {
    auto arguments = structopt::app("test").parse<ArrayOptionalArgument>(
      std::vector<std::string>{"./main", "-i", "4", "5", "6"});
  ...
  {
    auto arguments = structopt::app("test").parse<ArrayOptionalArgument>(
      std::vector<std::string>{"./main", "-i", "0x53", "071", "0b01101111"});
