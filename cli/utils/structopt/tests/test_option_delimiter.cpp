...
TEST_CASE(
  "structopt can parse optional args with delimiters like `=` and `:`"
  * test_suite("option_delimiter"))
{
  ...
  {
    auto arguments = structopt::app("test").parse<GccOptions>(std::vector<std::string>{
      "./gcc", "-v", "-Wall", "-std=c++17", "main.cpp", "-o", "main"});
    REQUIRE(arguments.std.value() == "c++17");
  ...
  {
    auto arguments = structopt::app("test").parse<GccOptions>(std::vector<std::string>{
      "./gcc", "main.cpp", "--verbose", "--Wall", "-std:c++17", "--output:main"});
    REQUIRE(arguments.std.value() == "c++17");
    REQUIRE(arguments.verbose == true);
    REQUIRE(arguments.Wall == true);
    REQUIRE(arguments.Compile == false);
