...
TEST_CASE(
  "structopt can parse combined optional flag arguments"
  * test_suite("combined_optional"))
{
  {
    auto arguments = structopt::app("test").parse<FlagArguments>(
      std::vector<std::string>{"./main", "-abc"});
...
TEST_CASE(
  "structopt can parse combined optional and flag arguments"
  * test_suite("combined_optional"))
{
  {
    auto arguments = structopt::app("test").parse<FlagAndOptionArguments>(
      std::vector<std::string>{"./main", "-ac", "3.14", "2.718"});
    REQUIRE(arguments.a == true);
    REQUIRE(arguments.b == false);
