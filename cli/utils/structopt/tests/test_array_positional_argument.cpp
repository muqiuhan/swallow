...
TEST_CASE(
  "structopt can parse fixed array positional argument" * test_suite("array_positional"))
{
  // Array of ints
  {
  ...
  {
    auto arguments =
      structopt::app("test").parse<ArrayBoolArgument>(std::vector<std::string>{
  ...
  // 0 arguments provided
  {
    ...
    try
      {
        structopt::app("test").parse<ArrayIntArgument>(
